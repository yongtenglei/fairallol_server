...
// comprehensive test
func TestAdjustedWinner(t *testing.T) {
	fmt.Println("===============Test Adjusted Winner=================")
	dataPath := "../data"
	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".txt" {
			...
			if len(parts) >= 4 {
				...
				testAdjustedWinner(dataFile)
			}
		}
	}
}

func testAdjustedWinner(fileData data.DataFile) {
...
func TestShallow(t *testing.T) {
	fmt.Println("===============Shallow Test adjusted Winner=================")
	dataPath := "../data"
	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".txt" {
			...
			if len(parts) >= 3 {
				...
				testAdjustedWinnerShallow(dataFile)
			}
		}
	}
}

func testAdjustedWinnerShallow(fileData data.DataFile) {
	tracker := data.Tracker{
		ScoresDiff: make([]int, 0),
		NumCases:   len(fileData.FileData),
	}
