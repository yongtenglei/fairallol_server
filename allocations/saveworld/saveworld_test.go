	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".txt" {
			...
			if len(parts) >= 4 {
				...
				testSaveWorld(dataFile)
			}
		}
	}
}

func testSaveWorld(fileData data.DataFile) {
	...
	for i, j := 0, 1; i < len(cases); i, j = i+2, j+2 {
		...
		allocation, _ := saveworld(agents, items)
...
func TestShallow(t *testing.T) {
	fmt.Println("===============Shallow Test SaveWorld=================")
	dataPath := "../data"
	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".txt" {
			...
			if len(parts) >= 3 {
				...
				testSaveWorldShallow(dataFile)
			}
		}
	}
}

func testSaveWorldShallow(fileData data.DataFile) {
	tracker := data.Tracker{
		ScoresDiff: make([]int, 0),
		NumCases:   len(fileData.FileData),
	}
