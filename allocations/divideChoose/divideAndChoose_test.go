...
// comprehensive test
func TestDivideAndChoose(t *testing.T) {
	fmt.Println("===============Test Divide and Choose=================")
	dataPath := "../data"
	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".txt" {
			...
			if len(parts) >= 4 {
				...
				testDivideAndChoose(dataFile)
			}
		}
	}
}

func testDivideAndChoose(fileData data.DataFile) {
...
func TestShallow(t *testing.T) {
	fmt.Println("===============Shallow Test Divide and Choose=================")
	dataPath := "../data"
	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".txt" {
			...
			if len(parts) >= 3 {
				...
				testDivideAndChooseShallow(dataFile)
			}
		}
	}
}

func testDivideAndChooseShallow(fileData data.DataFile) {
	tracker := data.Tracker{
		ScoresDiff: make([]int, 0),
		NumCases:   len(fileData.FileData),
	}
